...
function startAnimation() {
  var movtIzq = new TWEEN.Tween(eje.position).to({x:[-1.5,-2.5], y:[0,0], z:[0,2.5]}, 5000).interpolation(TWEEN.Interpolation.Bezier).easing(TWEEN.Easing.Bounce.Out);
  var movtFrente = new TWEEN.Tween(eje.position).to({x:[0,2.5], y:[0,0], z:[0,2.5]}, 5000).interpolation(TWEEN.Interpolation.Bezier).easing(TWEEN.Easing.Bounce.Out);
  var movtDer = new TWEEN.Tween(eje.position).to({x:[1.5,2.5], y:[0,0], z:[0,-2.5]}, 5000).interpolation(TWEEN.Interpolation.Bezier).easing(TWEEN.Easing.Bounce.Out);
  var movtTras = new TWEEN.Tween(eje.position).to({x:[0,-2.5], y:[0,0], z:[-1.5,-2.5]}, 5000).interpolation(TWEEN.Interpolation.Bezier).easing(TWEEN.Easing.Bounce.Out);
  movtIzq.chain(movtFrente);
  movtFrente.chain(movtDer);
  movtDer.chain(movtTras);
  // movtTras.chain(movtIzq);
  movtIzq.start();

  var giro = new TWEEN.Tween(eje.rotation).to({x:0, y:Math.PI*2, z:0}, 10000);
...
function update(){
  ...
  angulo += vueltasXsg * Math.PI*2 * (ahora-antes)/1000; // radianes por segundo
