...
(define (domain puerto_temp)
    ...
    (:durative-action desapilarObjetivo
        ...
        :duration (= ?duration (/ (- 10 (altura ?p)) (/ 3 (peso ?c))))
	    :condition 
            (and 
                (over all (en ?l ?g))
                (over all (en ?l ?p))
    ...
    ;
    (:durative-action moverCinta
        :parameters (?l1 - muelle ?l2 - muelle ?k - cinta ?c - caja)
        :duration   (= ?duration (/ (distancia ?l1 ?l2) (velocidad ?k)))
        :condition 
            (and 
